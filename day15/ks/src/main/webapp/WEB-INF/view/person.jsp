...
<body class="gray-bg">
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<td>人员爱好：</td>
								<td>
									<input type="checkbox" id="pLike1" name="pLike"  aria-required="true" required value="名表"/>名表
									<input type="checkbox" id="pLike2" name="pLike"  aria-required="true" required value="豪车"/>豪车
							...
							<td>人员年龄：</td>
								...
								<td>&nbsp;&nbsp;</td>
								<td>存款日期：</td>
								<td><input placeholder="存款日期" id="createtime" name="createtime" class="laydate-icon form-control layer-date"/></td>
								<td>&nbsp;&nbsp;</td>
							</tr>
							<tr>
							<td>存款金额：</td>
	...
	<!-- 新增和修改对话框 -->
	<div class="modal fade" id="modal_user_bank" role="dialog" aria-labelledby="modal_user_bank" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_bank" method="post" action="reserveBank.htm">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_bank_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    //添加部门弹
	    $("#submit_form_bank_btn").click(function(){
	    	$("#form_bank").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addBank").click(function(){
	    		$('#modal_user_bank').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_bank').modal('show');
	        });
	    	...
	    	$("#btn_echarts").click(function(){
    			...
    			$.ajax({
    			    ...
    			    success:function(res){
    			    	...
    			    	if(res.success){
    		    			//循环集合
    		    			res.elist.forEach(function(obj){
    		    				xdata.push(obj.bname);
    		    				ydata.push({value:obj.num,name:obj.bname});
    		    			});
    		    			//设置图表  饼状图
    		    			option = {
	    			    		    title: {
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+getSelection.pSex+"']").prop("checked",true);//性别   
		$("#pLike1").prop("checked",getSelection.pLike.indexOf("名表")!=-1);
		$("#pLike2").prop("checked",getSelection.pLike.indexOf("豪车")!=-1);
