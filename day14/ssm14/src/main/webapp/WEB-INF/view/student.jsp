...
<body class="gray-bg">
	...
	<!-- 添加专业对话框 -->
	<div class="modal fade" id="modal_user_teacher" role="dialog" aria-labelledby="modal_user_teacher" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_teacher" method="post" action="addTeacher.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>老师名称：</td>
								<td><input type="text" id="name" name="name"
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_teacher_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    var validator = $("#form_teacher").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	    ...
	    $("#submit_form_teacher_btn").click(function(){
	    	$("#form_teacher").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_teacher").click(function(){
	    			$('#modal_user_teacher').modal({backdrop: 'static', keyboard: false});
					$('#modal_user_teacher').modal('show');
	    	...
	    	$("#btn_echarts").click(function(){
	    			...
	    			$.ajax({
	    			    ...
	    			    success:function(res){
	    			    	var xdata = new Array();
	    			    	var ydata = new Array();
	    			    	res.li.forEach(function(student){
	    			    		xdata.push(student.tname)
	    			    		ydata.push({key:student.num,name:student.tname})
	    			    	})
	    			    	if(res.success){
	    			    		option = {
	    			    			    title: {
